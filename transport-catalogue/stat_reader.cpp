...
void statreader::StatReader::ParseAndPrintStat(transport_catalogue::processing::TransportCatalogue& transport_catalogue,
	std::string_view request)
{
	if (request[0] == 'B') {
		...
		double geo_length = 0;
		int road_length = 0;
		std::unordered_set<const domain::Stop*> unique(bus->stops_.begin(), bus->stops_.end());
		unique_stops = unique.size();
		total_stops = bus->stops_.size();

		for (unsigned i = 0; i < total_stops - 1; ++i)
		{
			geo_length += ComputeDistance(bus->stops_.at(i)->coord_, bus->stops_.at(i + 1)->coord_);
			road_length += transport_catalogue.FindDist(bus->stops_.at(i), bus->stops_.at(i + 1));
		}


		*os_ << "Bus " << busname << ": " << total_stops
			<< " stops on route, " << unique_stops << " unique stops, "
			<< std::setprecision(6) << static_cast<double>(road_length) << " route length, "
			 << static_cast<double>(road_length)/geo_length
			<<" curvature\n";
	}
	else 
	{
...
void statreader::StatReader::StartRequestParcing(transport_catalogue::processing::TransportCatalogue& tansport_catalogue)
{
